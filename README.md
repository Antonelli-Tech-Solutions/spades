...
- **Lobby** (`#/lobby`) — main menu after login; shows options to create or join a table and a collapsible Info Panel with tabbed Friends and Game History views; the Friends tab shows presence badges (online / in-game / offline) and polls `GET /api/friends` every 30 seconds; redirects back to the game screen if the player is already seated
