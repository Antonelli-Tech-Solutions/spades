...
- **Lobby** (`#/lobby`) — main menu after login; shows options to create or join a table and a collapsible Info Panel with tabbed Friends and Game History views; the Friends tab shows presence badges (online / in-game / offline), a Pending Requests section with Accept/Decline buttons for incoming friend requests, and a count badge on the tab when requests are pending; polls `GET /api/friends` every 30 seconds; redirects back to the game screen if the player is already seated
...
client/
  web/
    ...
    src/
      ...
      infoPanel.js    — Collapsible tabbed Info Panel (Friends + Game History tabs), viewable on all screens
