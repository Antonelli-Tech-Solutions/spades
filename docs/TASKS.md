...
- [x] `BUG` Fix trick count double-counted on non-playing clients (issue #296): `TRICK_COMPLETE` payload now includes the server's authoritative `tricksWon` object; the client `applyDelta` handler SETs this value instead of incrementing, making the operation idempotent. Applying the event twice (e.g. during a WS reconnect buffer replay) now produces the correct result instead of double-counting the trick.
